...
async function scheduleAnalysisNotification() {
  await Notifications.scheduleNotificationAsync({ content: { title: 'SuperEnalotto Analyzer', body: 'Nuova analisi disponibile! Scopri i top 9 numeri.' }, trigger: { seconds: 2 } });
}
...
function TermsScreen({ navigation }) {
  ...
  const handleAccept = async () => {
    ...
    if (isMinor) { Alert.alert('Attenzione', 'Come minorenne, puoi usare l\'app solo per scopi statistici. Il gioco d\'azzardo e vietato ai minori di 18 anni.', [{ text: 'Ho capito', onPress: () => navigation.replace('Login') }]); }
    else { navigation.replace('Login'); }
  };
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      <View style={[styles.termsCard, { backgroundColor: theme.card }]}>
        ...
        <Text style={[styles.termsText, { color: theme.subtext }]}>Questa app fornisce analisi statistiche sui numeri del SuperEnalotto a scopo informativo. Non garantisce vincite.</Text>
        <Text style={[styles.termsSubtitle, { color: theme.text }]}>2. Gioco Responsabile</Text>
        <Text style={[styles.termsText, { color: theme.subtext }]}>Il gioco d'azzardo puo causare dipendenza. Gioca in modo responsabile. Numero verde: 800 558 822.</Text>
        <Text style={[styles.termsSubtitle, { color: theme.text }]}>3. Eta minima</Text>
        <Text style={[styles.termsText, { color: theme.subtext }]}>Il gioco d'azzardo e vietato ai minori di 18 anni. I minorenni possono usare l'app solo per scopi educativi.</Text>
        <Text style={[styles.termsSubtitle, { color: theme.text }]}>4. Privacy</Text>
        <Text style={[styles.termsText, { color: theme.subtext }]}>I tuoi dati sono usati solo per l'accesso. Non condividiamo dati con terze parti.</Text>
        <Text style={[styles.termsSubtitle, { color: theme.text }]}>5. Responsabilita</Text>
        <Text style={[styles.termsText, { color: theme.subtext }]}>L'app non si assume responsabilita per perdite economiche. Le analisi sono basate su statistiche.</Text>
      </View>
      <View style={[styles.ageCard, { backgroundColor: theme.card }]}>
        ...
        <View style={styles.ageRow}>
          <TouchableOpacity style={[styles.ageButton, { backgroundColor: isMinor === false ? '#4caf50' : theme.badge }]} onPress={() => setIsMinor(false)}><Text style={[styles.ageButtonText, { color: isMinor === false ? '#fff' : theme.text }]}>✅ Maggiorenne</Text><Text style={[styles.ageSubtext, { color: isMinor === false ? '#e0e0e0' : theme.subtext }]}>18+</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.ageButton, { backgroundColor: isMinor === true ? '#ff9800' : theme.badge }]} onPress={() => setIsMinor(true)}><Text style={[styles.ageButtonText, { color: isMinor === true ? '#fff' : theme.text }]}>🔞 Minorenne</Text><Text style={[styles.ageSubtext, { color: isMinor === true ? '#e0e0e0' : theme.subtext }]}>&lt;18</Text></TouchableOpacity>
        </View>
      </View>
...
function ExtractionListScreen() {
  ...
  return (
    <ScrollView style={[styles.container,{backgroundColor:theme.bg}]}>
      <View style={[styles.listHeader,{backgroundColor:theme.card}]}><Text style={[styles.sectionTitle,{color:theme.text}]}>📋 Archivio Estrazioni</Text></View>
